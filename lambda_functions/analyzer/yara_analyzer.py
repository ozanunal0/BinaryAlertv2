...
class YaraAnalyzer:
    ...
    def _yextend_matches(self, target_file: str) -> list[YaraMatch]:
        ...
        """
        try:
            # Assuming yextend is in the system PATH
            ...
            return []
             ...
             LOGGER.error('Yextend executable not found in PATH')
             return []

        try:
            decoded_output = output.decode('utf-8')
        except UnicodeDecodeError:
            LOGGER.error(f'Yextend output could not be decoded to utf-8:\n{output}')
            return []
        ...
        try:
            ...
            try:
                ...
                LOGGER.error(f'Cannot parse yextend output as JSON:\n{decoded_output}')
                return []
        ...
        try:
            ...
            LOGGER.error(f'Yextend output: {decoded_output}')
            return []
    ...
    def analyze(self, target_file: str, original_target_path: str = '') -> list[YaraMatch]:
        ...
        try:
            ...
            # Assuming upx is in the system PATH
            subprocess.check_output(['upx', '-q', '-d', target_file], stderr=subprocess.STDOUT)
            LOGGER.info(f'Unpacked UPX-compressed file {target_file}')
        except subprocess.CalledProcessError:
            pass  # Not a packed file
        except FileNotFoundError:
            LOGGER.warning('UPX executable not found in PATH, skipping unpacking')
        ...
        for match in raw_yara_matches:
            ...
            for string_match in match.strings:
                string_names.add(string_match.identifier)
                for instance in string_match.instances:
                    try:
                        string_data.add(instance.matched_data.decode('utf-8'))
                    except UnicodeDecodeError:
                        # Bytes string is not unicode - print its hex values instead
                        string_data.add(instance.matched_data.hex())
