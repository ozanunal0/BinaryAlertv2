...
# Expects a binary YARA rules file to be at '/var/task/rules/compiled_yara_rules.bin'
import json
import os
import urllib.parse
...
# In the Docker container, we bake the rules into a fixed location.
COMPILED_RULES_FILEPATH = '/var/task/rules/compiled_yara_rules.bin'
...
LOGGER = Logger(service="BinaryAlertAnalyzer")
...
def _objects_to_analyze(event: dict[str, Any]) -> Generator[tuple[str, str], None, None]:
    ...
    if 'Records' not in event:
        LOGGER.warning('Event does not contain Records', extra={"event": event})
        return
    ...
    for sqs_message in event['Records']:
        try:
            ...
            LOGGER.exception('Skipping invalid SQS message', extra={"sqs_message": sqs_message})
            continue
...
def analyze_lambda_handler(event: dict[str, Any], lambda_context: Any) -> dict[str, Any]:
    ...
    """
    # Note: System tools (upx, pdftotext) are expected to be in the global PATH in the Docker image.
    ...
    for bucket_name, object_key in _objects_to_analyze(event):
        LOGGER.info(f'Analyzing "{bucket_name}:{object_key}"')
        ...
        try:
            ...
            LOGGER.exception(f'Unable to download {object_key} from {bucket_name}')
            continue

        if binary.yara_matches:
            LOGGER.warning(f'{binary} matched YARA rules: {binary.matched_rule_ids}')
            ...
            LOGGER.info(f'{binary} did not match any YARA rules')
            if alerts_enabled and os.environ.get('NO_MATCHES_SNS_TOPIC_ARN'):
